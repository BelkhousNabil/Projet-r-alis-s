<?php
	...
	$sql1="select id_desc from component where name='".$idf."'";
	...
	if($idf != $name_mid){
		...
		if($count==0){
			...
			$sqlNameRest="select restricted_name from component where name ='".$idf."'";
			...
			$updateMidd = $dbh->query("update component set name='$name_mid', id_desc='$id' , restricted_name = '$rest_name', description = '$description', contact = '$contacts', tech_contact = '$tech_cont', tech_desc = '$tech_desc' where name ='".$idf."'");

			// get the name of the flow that contain this component 1
			$sqlNameflw="select name from flows where component_1 ='".$idf."'";
			...
			while($nameFlw = $resultnameFlw->fetch()){
				...
				$updateFlows = $dbh->query("update flows set component_1 = '$name_mid' where component_1 ='".$idf."'");
			}

			// get the name of the flow that contain this component 2
			$sqlNameflw="select name from flows where component_2 ='".$idf."'";
			$resultnameFlw = $dbh->query($sqlNameflw);
			while($nameFlw = $resultnameFlw->fetch()){
				// modify the flow name
				$newFlwName = str_replace($name_rest_old, $rest_name, $nameFlw[0]);
				...
				// update the the flows 
				$updateFlows = $dbh->query("update flows set component_2 = '$name_mid' where component_2 ='".$idf."'");
			}
			...
			$updateDesc = $dbh->query("update description set idenv=$environ,localisation='$loc',ipadr='$ip', dns='$dns', server='$server',compte='$access' where iddesc ='".$id."'");
			//update the component
			$updateMidd = $dbh->query("update component set id_desc='$id' , description = '$description', contact = '$contacts', tech_contact = '$tech_cont', tech_desc = '$tech_desc' where name ='".$idf."'");
