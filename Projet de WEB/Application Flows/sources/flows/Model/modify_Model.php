<?php
	...
	//Select the id description of the middleware
	$sql1="select id_desc from middleware where name='".$name."'";
	...
	if($name != $name_mid){
		...
		if($count==0){
			...
			$sqlNameRest="select restricted_name from middleware where name ='".$name."'";
			...
			$updateMidd = $dbh->query("update middleware set name='$name_mid', id_desc='$id' , restricted_name = '$rest_name', description='$description',client='$client',users='$users', contact_client='$contact_cl', service='$service', appli_manager='$appli_manager', appli_managment='$appli_team', prod_team='$prod_team', tech_desc='$tech_desc' where name ='".$name."'");

			// get the name of the flow that contain this middleware
			$sqlNameflw="select name from flows where middleware ='".$name."'";
			...
			while($nameFlw = $resultnameFlw->fetch()){
				...
				$updateFlows = $dbh->query("update flows set name = '$newFlwName', middleware = '$name_mid' where middleware ='".$name."'");
			}
