<?php
	  ...
	  /**
        ajaxDestination_refresh.php 
            description --> This page permits to have the autocompletion feature when we search the flows by destination
            Controllers --> None
            Model --> None
    **/
...
$sql = "SELECT name FROM partner WHERE name LIKE (:keyword) ORDER BY name";
...
foreach ($list as $rs) {
	...
	// add new option
    echo '<li onclick="set_item_part2(\''.str_replace("'", "\'", $rs['name']).'\')">'.$name.'</li>';
}
?>