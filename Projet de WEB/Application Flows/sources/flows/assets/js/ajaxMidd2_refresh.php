...
$sql = "SELECT distinct name FROM middleware WHERE name LIKE (:keyword) ORDER BY name";
...
foreach ($list as $rs) {
	...
	// add new option
    echo '<li onclick="set_item_mid(\''.str_replace("'", "\'", $rs['name']).'\')">'.$name.'</li>';
}
?>