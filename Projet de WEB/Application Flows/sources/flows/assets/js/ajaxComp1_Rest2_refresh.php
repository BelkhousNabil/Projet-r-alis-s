<?php
	  ...
	  /**
        ajaxComp1_Rest2_refresh.php 
            description --> This page permits to have the autocompletion feature when we search the seconde component on a flow
            Controllers --> None
            Model --> None
    	**/
...
$sql = "SELECT distinct name FROM component WHERE name LIKE (:keyword) ORDER BY name";
...
foreach ($list as $rs) {
	...
	// add new option
    echo '<li onclick="set_item_comp1(\''.str_replace("'", "\'", $rs['name']).'\')">'.$name.'</li>';
}
?>