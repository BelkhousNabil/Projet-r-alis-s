...
class Neuron:
    ...
    def chooseConnectedNeuron(self,shift):
        ...
        tmpConnection = dict(self.connections)
        dif = self.index - shift
        while neuron.testNeuron(dif) == False :
            tmpConnection.pop(neuron)
            neuron = self.weighted_choice(tmpConnection)
        return neuron
